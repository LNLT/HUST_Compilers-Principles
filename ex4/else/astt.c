...
{ //显示符号表
    ...
    for (i = 0; i < myTable.index; i++)
        printf("%6s %6s %6d  %6s %4c %6d\n", myTable.symbols[i].name,
               ...
               myTable.symbols[i].type == INT ? "int" : "float",
...
//             rtn = fillmyTable(T->type_id, newAlias(), LEV, T->type, 'A', T->offset);//偏移量为0
// 			if (rtn == -1) {
//                 semantic_error(T->pos,T->type_id, "变量重复定义");
//             }
//             else {
...
//                     opn1.kind=ID; strcpy(opn1.id,myTable.symbols[T->ptr[0]->place].alias);
//                     opn1.offset=myTable.symbols[T->ptr[0]->place].offset;
//                     opn2.kind=ID; strcpy(opn2.id,myTable.symbols[T->ptr[1]->place].alias);
...
{ //对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
    ...
    if (T)
    {
        ...
        case TYPE:
            ...
            break;
        case VOID_FUNC_DEF:
            T->ptr[1]->type = "void"; //获取函数返回类型送到含函数名、参数的结点
