...
//         printf("%6d\t", myTable.symbols[i].level);
...
//             printf("%6d", myTable.symbols[i].paramnum);
...
// int fillSymbolTable(char *name, char *alias, int level, int type, char flag, int offset) // 首先根据name查符号表，不能重复定义 重复定义返回-1
// {
//     int i;
//     /*符号查重，考虑外部变量声明前有函数定义，
